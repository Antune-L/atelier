...
Every external mutation (git, tmux, `gh`/`az`, `osascript`, `~/.claude.json`, `bun install`, filesystem) goes through one injectable interface: `src/server/system/types.ts` (`SystemAdapter`). Two implementations:
...
- **`RealSystemAdapter`** — only when `KANBAN_DRY_RUN=0`. Actually runs git/tmux and the project's VCS CLI (`gh` or `az`). **Never instantiated in the dev/test path.**
...
- agent → backend: the worker tools (`update_stage`, `ask_user`, `submit_prd`, `submit_answer`, `done`, `ready_for_review`, `fail`, plus `submit_triage`/`submit_feasibility`, and `read_review_results` to re-read the verdicts persisted for the ticket's current review pass when a `review_done` event was lost) route to the coordinator through the provider's MCP connection. For Claude or Codex implementers, `delegate_implementation` starts backend-owned child sessions (max `MAX_PARALLEL_IMPLEMENTERS` concurrent lots, distinct labels and non-overlapping relative file scopes). Each scoped lot runs in its own worktree; integration rejects changes outside its scope or conflicting changes in the ticket worktree. Omitting `files` uses the shared ticket worktree and reserves the whole repository while that lot runs. The backend emits one `implementation_done` per lot with its label and remaining count. A failed lot may be started once more with the same label and scope, for at most two starts per label in a fresh execution cycle. Composer keeps its separate script path. Security posture is `permissionMode: 'dontAsk'` + an explicit allow-list (bash patterns; non-bash safe tools), an in-process PreToolUse hook denying `--no-verify`, and `permissions.deny` for read-only triage/feasibility scouts.

**Slots.** A fixed pool (`SLOT_COUNT`, default 5, override `KANBAN_SLOTS`) of git-worktree slots at `SLOTS_ROOT/slot-N` (`~/kanban-worktrees/`, outside any repo so tsc in a slot never inherits parent `node_modules/@types`). `SlotManager` lifecycle: cleanup → fetch → `worktree add -b` → copy env → install → start provider session → inject contract → `done` gate → release. Git ops per repo are serialized through `repoMutex`. No free slot → FIFO queue. A dead turn is auto-reclaimed in place (worktree preserved) up to `AUTO_RECLAIM_MAX`, then fails; a lost session at backend restart relaunches active-stage slots in place. A read-only review pass resets its worktree (`git checkout -- .` + `git clean -fd`, ignored files kept) before checking it is clean; fix-mode reviews never reset.

**The `done` gate is server-verified.** `done(pr_url)` does not trust the agent: the backend re-checks (clean working tree, branch pushed, PR exists via the project's `VcsClient`). Failure → `stalled`, slot kept. The SDK turn-end event escalates a turn that ended without any protocol tool: auto-nudge once (`nudge` event) → otherwise `stalled` + notification.

**Column vs Stage are different axes.** `COLUMNS` (10: todo/implementing/prd/to_review/done/merged/reviewed/answered/failed/abandoned) is the board lane; `STAGES` (12: queued…opening_pr/done/failed/interrupted/stalled) is the fine-grained pipeline state. Use `TERMINAL_STAGES` / `ACTIVE_STAGES` rather than hardcoding. Ticket `kind` is `feature` | `review` | `ask` | `clean` (see `KINDS` in `src/shared/constants.ts`): a review ticket runs argus on a PR, a clean ticket triages and fixes PR feedback, an ask ticket answers a read-only question as a comment.

**The VCS seam is complete on both providers.** Review publication and the PR-feedback clean flow run on GitHub and on Azure DevOps alike, so no flow is refused at creation time for its provider any more. On Azure, `publish_review` posts one inline thread per finding anchored on the latest iteration's `changeTrackingId`, and the last thing it posts carries the pass's completion marker and is the persisted `reviewId`: a general summary thread when the body is non-empty, else the last inline thread, else a closed marker-only thread (so no empty comment shows); it also maps the verdict to `az repos pr set-vote` (`approve` / `wait-for-author` / no vote for COMMENT); a refused vote degrades instead of failing, like the GitHub self-approval downgrade. Unanchorable findings fold into the summary body through the shared `reviewMarkdown.ts` renderer. The clean flow reads and resolves threads with `az devops invoke` (`GET`, then `PATCH {"status":"fixed"}`); `Bash(az devops invoke:*)` is allow-listed for Azure **clean** sessions only — never for review sessions, where `reviewPublishingGuard.ts` denies every mutating `az` form. `argus --post` and the `minos-pr-feedback` skill drive the GitHub API and stay GitHub-only: an Azure session gets the equivalent `az` commands inline on both agent providers, and the skill is not listed for it. Azure limits: cloud only, squash instead of rebase on merge, no additions/deletions on a PR payload. Details and the write paths never exercised against a live organisation: `docs/azure-devops-support-state.md`.
...
- **Documentation is English.** Every Markdown file (`README.md`, `docs/`, `AGENTS.md`, `CONTEXT.md`), every code comment, every CI step label, every example-config string and the whole public MCP surface (`publicMcp.ts` / `ticketOperations.ts`: tool and schema descriptions plus client-facing errors) is written in English. Write new ones in English and translate any French you touch. This covers *documentation and the public API contract* only — **internal runtime** strings (product UI in `src/web`, agent prompts in `src/server/agents`, log/error messages elsewhere in `src/server` and `desktop`) stay as they are; several are asserted verbatim by tests, so translating them is a behaviour change, not a docs change. Accent-based greps miss unaccented French (`mon-projet`, `le`, `des`) — read the strings, don't only grep them. Known consequence: `ticketOperations.ts` backs both the MCP tools and the REST routes, so its English messages also surface in the French board UI next to French ones from the same handler (`routes.ts`). The API contract wins — don't "fix" that by translating them back.
